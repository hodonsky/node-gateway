...
export default class Actor {
   ...
   */
  #buildResponder( correlationId, resolve, reject, responseAVRO, errorAVRO ) {
    this.#responders[ correlationId ] = ({ content, error: responderError }) => {
      let response, error
      try {
        response = fromAVRO( content, responseAVRO, { response: true } )
      } catch ( err ) {
        console.debug( err )
        error = fromAVRO( content, errorAVRO, { error: true } )
      } 
      if ( error ) {
   ...
   */
  createRequest( { action, requestAVRO, responseAVRO, errorAVRO }, data ) {
    /**
    ...
    if ( !this.#link ) {
      return new Promise( ( resolve ) =>
        setTimeout(() =>
          process.nextTick( () =>
            resolve( this.createRequest( { action, requestAVRO, responseAVRO, errorAVRO }, data ) )
          ), 500 ) )
    }
    const correlationId = this.#config.uuidFn()
    return new Promise( async ( resolve, reject ) => {
      try {
        this.#buildResponder( correlationId, resolve, reject, responseAVRO, errorAVRO )
        try {
