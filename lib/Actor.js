...
export default class extends Base {
  ...
  async #initilizeConnection(){
    ...
    const conn = new Connector().queueConnection()

    console.debug("Initilizing Connection")
    const detach = () => {
      this.#link?.close()
      this.#link = null
    }
    const attach = async connection => {
      connection.on( "error", error => ( detach(), this.emit( "error", error ) ) )
      connection.on( "close", close => ( detach(), this.emit( "close", close ) ) )

      this.#link = await connection.createConfirmChannel()
      this.#link.assertQueue( this.#responseTopic, { durable: true, autoDelete: true } )
      this.#link.consume(
        this.#responseTopic,
  ...
  constructor( config ) {
    ...
    this.on( "close", err => console.info( err ) )
    this.on( "processKill", async () => await this.#deconstruct())
  }
  async #deconstruct(){
    await this.#link.deleteQueue( this.#responseTopic )
